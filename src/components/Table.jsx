...
export default function Table({dvds,handleDelete})
{
    return(
        <>
    ...
    <table className="w-full text-sm text-left text-gray-500 dark:text-gray-400">
        ...
        <tbody>
                       {dvds.map((dvd, index) =>{ 
                        const dvdId=dvd.id;                        
                        return(
          <tr key={index} className="bg-white border-b dark:bg-green-800 dark:border-green-700">
            <td className="px-6 py-4"><Link  to={"/dvd/"+dvdId}>{dvd.title}</Link></td>
